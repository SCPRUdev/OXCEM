		...
		try
		{
			if (isListHelper(reader))
			{
				...
				for (const auto& n : reader.children())
				{
					if (isFirst)
					{
						// Allows to reset Collections::NamesToIndex. Use it if you really-really
						// need to reset it. Exists in case you've decided to compare bitset
						// as value and require strict index order for it, but collection already
						// has data from other mods. Do note that you might need to override all
						// bitset-related values in all other ruleset entries that use it.
