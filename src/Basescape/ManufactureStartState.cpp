...
ManufactureStartState::ManufactureStartState(Base *base, RuleManufacture *item) :  _base(base), _item(item)
{
	...
	{
		// separator line
		_lstRequiredItems->addRow(1, tr("STR_MANUFACTURE_OUTPUT").c_str());
		_lstRequiredItems->setCellColor(row, 0, _lstRequiredItems->getSecondaryColor());
		row++;
...
{
	if (_item->getProducedCraft() && _base->getFreeCraftSlots(_item->getProducedCraft()->getRequiresCraftSlotFunc()) <= 0)
	{
