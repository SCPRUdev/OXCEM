...
{
	if (change <= 0) return;
	if (_production->getRules()->getProducedCraft() && _base->getFreeCraftSlots(_production->getRules()->getProducedCraft()->getRequiresCraftSlotFunc()) <= 0)
	{
		_timerMoreUnit->stop();
	...
	else
	{
		...
		if (_production->getRules()->getProducedCraft())
			change = std::min(_base->getFreeCraftSlots(_production->getRules()->getProducedCraft()->getRequiresCraftSlotFunc()), change);
